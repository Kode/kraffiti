...
void writeBMPHeader(FILE *file, int width, int height) {
	...
	write(file, convertShortToByteArrayLE(
	                (short)32));             // the number of bits per pixel, which is the color depth of the image. Typical values are 1, 4, 8, 16, 24 and 32.
	write(file, convertIntToByteArrayLE(0)); // the compression method being used. See the next table for a list of possible values.
	write(file, convertIntToByteArrayLE(
	                width * height * 4)); // the image size. This is the size of the raw bitmap data (see below), and should not be confused with the file size.
...
void writeBMPIcon(FILE *file, Image image) {
...
void writeBMPFileHeader(FILE *file, int width, int height) {
	fputc(66, file);
	fputc(77, file);
	write(file, convertIntToByteArrayLE(bmpHeaderSize + 4 + 2 + 4 * width * height));
	write(file, convertIntToByteArrayLE(0));
	write(file, convertIntToByteArrayLE(54));

	write(file, convertIntToByteArrayLE(bmpHeaderSize)); // the size of this header (40 bytes)
	write(file, convertIntToByteArrayLE(width));         // the bitmap width in pixels (signed integer).
	write(file, convertIntToByteArrayLE(height));        // the bitmap height in pixels (signed integer).
	write(file, convertShortToByteArrayLE((short)1));    // the number of color planes being used. Must be set to 1.
	write(file, convertShortToByteArrayLE(
	                (short)24));             // the number of bits per pixel, which is the color depth of the image. Typical values are 1, 4, 8, 16, 24 and 32.
	write(file, convertIntToByteArrayLE(0)); // the compression method being used. See the next table for a list of possible values.
	write(file, convertIntToByteArrayLE(
	                width * height * 3)); // the image size. This is the size of the raw bitmap data (see below), and should not be confused with the file size.
	write(file, convertIntToByteArrayLE(0)); // the horizontal resolution of the image. (pixel per meter, signed integer)
	write(file, convertIntToByteArrayLE(0)); // the vertical resolution of the image. (pixel per meter, signed integer)
	write(file, convertIntToByteArrayLE(0)); // the number of colors in the color palette, or 0 to default to 2n.
	write(file, convertIntToByteArrayLE(0)); // the number of important colors used, or 0 when every color is important; generally ignored.
...
void writeBMP(FILE *file, Image image) {
	writeBMPFileHeader(file, image.width, image.height);
	for (int y = image.height - 1; y >= 0; --y) {
		for (int x = 0; x < image.width; ++x) {
			// stream.put(image->b(x, y));
			// stream.put(image->g(x, y));
...
void windowsIcon(Image image, const char *filename) {
	...
	iconOffset += getBMPSize(16, 16);
	writeIconDirEntry(file, 20, 20, iconOffset);
	iconOffset += getBMPSize(20, 20);
	writeIconDirEntry(file, 24, 24, iconOffset);
	iconOffset += getBMPSize(24, 24);
	writeIconDirEntry(file, 32, 32, iconOffset);
	iconOffset += getBMPSize(32, 32);
	writeIconDirEntry(file, 40, 40, iconOffset);
